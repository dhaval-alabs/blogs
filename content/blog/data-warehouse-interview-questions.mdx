...
featuredImage: "https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/dw-interview.webp"
author: AnalytixLabs
categories:
  - Big Data
  - Data Science
...
![data warehouse names](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/2.jpg)
...
![data warehouse architecture](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/3.jpg)
...
| **OLTP** | **OLAP** |
| --- | --- |
| The initial data source is operational data | The data source is the data consolidated from varied sources |
| The goal is to provide a snapshot of the business processes involved in the essential tasks | The goal is to provide a multidimensional view of the various business events for decision-making |
| Designed for running simple queries quickly | Designed for running complex long queries |
| Database design uses normalized and small databases. An entity-relationship (ER) model and function-oriented database design are adopted. | Database design uses denormalized and large databases. A Star, Snowflake, or Fact constellation mode of subject-oriented database design is adopted. |
| Speed is not an issue | Speed is an issue |

### Q6. **What are the common functions performed by OLAP?**

**Ans.** The key functions performed by OLAP include the following-

![olap functionalities](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/4.jpg)
...
| TRANSACTIONAL | The most common and basic type of fact table is a transaction fact table that indicates the occurrence of an event at any given time. It has one row per transaction and holds data at a highly detailed level, with several dimensions linked to it. Users can have extensive dimensional grouping and use features like roll-up, drill-down, etc., when using this type of fact table. Thus, a transaction fact table provides a fundamental view of a corporate process. |
| --- | --- |
| SNAPSHOT | A snapshot fact table is used if one has to provide the condition of things at a specific time. As the name suggests, it provides a “snapshot of the moment” to the organization. Organizations use such a table to examine their performance at different time intervals. A snapshot table differs from a transaction table as it contains non-additive and semi-additive information. Unlike a transaction fact table (where each occurrence of an event leads to the addition of a new row), a snapshot fact table represents the performance of activity at different (often regular) time intervals (such as the end of the day, week, month, etc.). However, these two tables are not mutually exclusive, and the snapshot table relies on the transaction fact table to retrieve detailed data. |
| ACCUMULATING | Accumulating fact tables provide a well-defined beginning and end to any activity or process. A table often has multiple data stamps reflecting the events or predictable stages occurring during the lifespan of the activity. Such tables often have a dedicated column with timestamps indicating the last time the row was updated. |
...
| --- | --- |
| Hold facts, metrics, and measurements of attributes | A complementary table to the fact table that is used by the fact table to derive facts |
| Defines facts at the atomic level | Detailed, lengthy, and comprehensive |
| Used for decision-making and analysis | Used for following the organization’s processes and operations |
| Doesn’t have a hierarchy | Has a hierarchy |
| Has lesser attributes | Has more attributes |
| Has more records | Has fewer records |
| Grows vertically | Grows horizontally |
| Created after the dimension table | Created before the development of the fact table |
...
| --- | --- |
| Uses Online Analytical Processing (OLAP) | Uses Online Transaction Processing (OLTP) |
| Used for the analysis of historical data to make data-backed decision-making | Uses for executing basic business procedures |
| Data is denormalized, enabling the joining of tables to be relatively easy | Tables and joins are complicated as they are normalized |
| Collection of data is subject-oriented | Collection of data is application-oriented |
| Data modeling techniques are deployed to design the data warehouse | Entity-relationship (ER) modeling is used when designing a database |
| Data is not always up-to-date | Data is typically up-to-date |
| Star and Snowflake schema is generally employed as the data structure based on the dimensional and normalized approach | Flat Relational Approach is employed for storing data |
| Data stored in a data warehouse can be highly summarized | Data stored in a database is generally detailed |
...
![star schema](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/star-schema.jpg)
...
![snowflake schema](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/snowflake-schema.jpg)
...
| --- | --- |
| Fact and Dimension tables are contained within the schema | Fact, dimension, and sub-dimension tables are contained within the schema |
| Follows a top-down model | Follows a bottom-up model |
| Query complexity is lower | Query complexity is higher |
| Uses more space | Uses less space |
| Easy to understand | Difficult to understand |
| Contains fewer foreign keys | Contains more foreign keys |
| Has relatively lot more redundancy | Has relatively low levels of redundancy |
| Normalization is not employed | Normalization and denormalization are employed |
| Follows a simple design | Follows a complicated design |
| Data retrieval is faster because of the simple centralized structure | Data retrieval is slower because of a complex decentralized structure |
...
![top down approach data warehouse](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/top-down-approach-data-warehouse.jpeg)
...
![bottom up approach data warehouse](https://tumfgdyurswklrzsnevv.supabase.co/storage/v1/object/public/uploads/wp-content/uploads/2024/01/bottom-up-approach-data-warehouse.jpg)
